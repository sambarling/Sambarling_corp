    ...
    : QMainWindow(parent)
{
    ...
    ui.groupBox->setGeometry(test); //инициализаци€ главного меню
    int tnas_cx;
    int tnas_cy;
    int del_cn;
    tnas_cx = cx / 10;
    ...
    hour = ltm->tm_hour;//инициализаци€ рандома
    if ((hour == 22) || (hour == 23))
    {
        hour = 0;
    }
...
void hariki2::start_game_slot()//окно игры
{
    ...
    cy = cy * dpix;
    QRect test(0, 0, cx, cy);
    ui_game.groupBox->setGeometry(test);//инициализаци€ игрового пол€
    ...
    time_t now = time(0);//ћодуль очистки игрового окна
    tm* ltm = localtime(&now);
    int secs = ltm->tm_sec;
    srand(secs);
    time_s = -10;
...
void hariki2::write_yes(int color, int tone, int stav)//запись правильной фигуры в пам€ть
{
    switch (stav)
    {
    case 0:
...
void hariki2::slotTimerAlarm()//перемешивание фигур
{
    if (pov == povt)
    {
        ...
        if (povt == 1) 
        {
            ...
            QString rezult(QString::fromLocal8Bit("—ледите за "));
            QString info;
...
void hariki2::name_but()//а можно ли вам играть смертный?
{
    ...
    {
        QMessageBox msgBox;  
        msgBox.setText(QString::fromLocal8Bit("ѕодождите пожалуйста.\n ¬ы недавно играли"));
...
void hariki2::DPI_botton()//DPI!!!!!!!!!!!!!
{
    ...
    if (rez_dpi_st == 100)
    {
        ui_test.test_rez->setText(QString::fromLocal8Bit("ћодуль DPI работает неправильно"));
